...
resource "aws_autoscaling_policy" "avg_cpu_policy_greater_than_xx" {
  ...
  estimated_instance_warmup = 180 # defaults to ASG default cooldown 300 seconds if not set . Tells AWS how long (in seconds) it should wait after a new instance launches before that instance’s metrics are considered in scaling decisions.
...
resource "aws_autoscaling_policy" "alb_target_requests_greater_than_yy" {
  name                      = "alb-target-requests-greater-than-xxxxx"
